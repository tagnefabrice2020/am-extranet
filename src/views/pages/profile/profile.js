...
const Profile = ({ authUser, fetchOneUser, users, updateUser }) => {
  let auth = JSON.parse(authUser.authUser);

  let role = auth.group.toLowerCase();
        ...
        async (value) => {
          ...
          if (id) {
            result = await axios.get(
              API_URL + `/manager_app/checker?email=${value}&id=${id}`
            );
...
.typeError("Veuillez saisir des characters numérique.");
  ...
  useEffect(() => {
    fetchOneUser(userId, auth.group.toLowerCase());
  }, [userId, fetchOneUser, auth.group]);
  ...
  const editProfile = async (data) => {
    const newData = role === CLIENT ? { ...data, is_active: true, role: "3" } : { ...data, is_active: true };
